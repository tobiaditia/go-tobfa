...
func (repository *BusinessRepositoryImpl) Get(ctx context.Context, tx *sql.Tx) []domain.Business {
	...
	for rows.Next() {
		...
		err := rows.Scan(&business.Id, &business.UserId, &business.Name, &business.BusinessCategoryId, &business.CountryId, &business.ProvinceId, &business.CityId, &business.DistrictId, &business.VillageId)
		helper.PanicIfError(err)
...
func (repository *BusinessRepositoryImpl) Create(ctx context.Context, tx *sql.Tx, business domain.Business) domain.Business {
	sql := "insert into businesses (user_id, name, address, business_category_id, country_id, province_id, city_id, district_id, village_id) values (?,?,?,?,?,?,?,?,?)"
...
func (repository *BusinessRepositoryImpl) Update(ctx context.Context, tx *sql.Tx, business domain.Business) domain.Business {
	sql := "update businesses set user_id = ?, name = ?, address = ?, business_category_id = ?, country_id = ?, province_id = ?, city_id = ?, district_id = ?, village_id = ? where id = ?"
